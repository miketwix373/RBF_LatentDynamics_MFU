...
def bic(n_k, M, J, N):
    """Bayesian information criterion for K-means, Colanera & Magri Eq. 18.

        BIC = M log(J) + K log(M) - (2/N) * Sum_k n_k log(n_k / M),

    with `J` the inner-cluster variance (their Eq. 8), i.e. the within-cluster
    sum of squares `Sum_m ||u_m - c_{beta(m)}||^2` that `kmeans_fit` returns as
    `inertia`, and `N` the ambient state dimension.

    The first term rewards tight clusters: `J` falls monotonically as `K`
    grows, so `M log(J)` decreases, traded off against the `K log(M)`
    complexity penalty. `K` is selected at the minimum of BIC, or - as
    Colanera do in practice (their Fig. 8) - at the elbow of the marginal
    variation `Delta BIC / Delta K`. The rightmost entropy term is `O(1/N)`
    and negligible for `N >> 1` (Colanera's remark), but kept for the low-N
    ODE testbeds where it is not (L63: `N=3`).

    The previous implementation transcribed the first term as `M log(M)`,
    which is constant in `K`; with the only `K`-decreasing term frozen the
    score became monotone increasing and selected `K=1` on every dataset.
    The fix is to pass `J` (the inertia) through, as Eq. 18 requires.
    ...
    J : float
        Inner-cluster variance (Eq. 8) = K-means `inertia` at this `K`.
    N : int
        Ambient state dimension (sets the `2/N` entropy weight).
    ...
    return float(M) * np.log(J) + float(K) * np.log(M) - (2.0 / float(N)) * entropy


def bic_eff(n_k, M, J, N, tau_int_steps):
    """Eq. 18 BIC with `M_eff = M / (2 * tau_int_steps)` for the sample count.

    Effective-sample correction proposed in
    `docs/notes/clustering-success-criterion.md` (Open question:
    BIC effective-sample correction); not in Colanera. The substitution
    replaces the sample count `M` in the `M log(J)` and `K log(M)` terms by
    the decorrelated count `M_eff`; the entropy term keeps `n_k` and `M`
    as-is because cluster populations are observables, not sample-count
    parameters. Both `bic` and `bic_eff` are reported by the sweep; the user
    picks `K` in a journal entry.
    ...
    entropy = float((n_k * np.log(n_k / M)).sum())
    return M_eff * np.log(J) + float(K) * np.log(M_eff) - (2.0 / float(N)) * entropy
...
#     bic             float64(L,)        Colanera Eq. 18, bic(n_k, M, J, N)
#     bic_eff         float64(L,)        bic_eff(n_k, M, J, N, tau_int_steps)
