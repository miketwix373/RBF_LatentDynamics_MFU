...
def rbf_features_wendland_mahal(
    A: np.ndarray,
    centers_std: np.ndarray,
    Sigma_invs_std: np.ndarray,
    *,
    ...
    """Wendland phi_{d_pd, k_smooth}(r_M / R_cut) on the Mahalanobis radius.

    Canonical Wendland C^{2 k_smooth} form (Wendland 1995, Adv. Comp. Math. 4;
    Schaback & Wendland 2006):
        l = floor(d_pd / 2) + k_smooth + 1
        phi(r) = (1 - r)_+^{l + k_smooth} * (poly_coef * r + 1),
        poly_coef = l + 1.
    For d_pd=3, k_smooth=1 this recovers phi(r) = (1-r)^4 (4r+1).

    Default d_pd=3 (NOT the embedding dimension of `A`) — used as a radial
    function of the d=35 Mahalanobis distance. The d_pd=3 form is broad
    enough to give substantive inter-centre overlap at k-NN-sized R_cut;
    the canonical d_pd=embedding_dim choice yields an (l+k)=20-degree
    polynomial that collapses to a numerical delta at any non-trivial
    R_cut. We use Wendland-3 as an RBF in d=35: PD-on-R^3 is sufficient
    for our regularised LSQ regression (Phi^T Phi + lam I is invertible
    for any lam > 0 regardless of kernel PD in the ambient space).
    rom-specialist 2026-06-18.
    """
    A_std = (A - mu_A) / sigma_A
    M = A_std.shape[0]
    n_c = centers_std.shape[0]
    ...
    exponent = l + k_smooth
    poly_coef = float(l + 1)
    out = np.zeros((M, n_c), dtype=A.dtype)
    inv_R_cut2 = 1.0 / (R_cut * R_cut)
    for j in range(n_c):
        d2 = _mahal_sq(A_std, centers_std[j], Sigma_invs_std[j])
        r2 = d2 * inv_R_cut2
        mask = r2 < 1.0
        if not mask.any():
            continue
        r_in = np.sqrt(r2[mask])
        one_minus_r = 1.0 - r_in
        out[mask, j] = (one_minus_r ** exponent) * (poly_coef * r_in + 1.0)
    return out


# ---------------------------------------------------------------------------
# Tikhonov ridge via SVD, and PRESS for shape-parameter LOOCV
# ---------------------------------------------------------------------------

def ridge_svd_factor(Phi_n: np.ndarray):
    """Thin SVD of the column-normalised feature matrix.

    Compute once, then sweep `lambda` cheaply through `ridge_svd_solve`
    and `ridge_svd_press`. Phi_n shape (M, n_c) with M >= n_c.
    """
    U, S, Vt = np.linalg.svd(Phi_n, full_matrices=False)
    return U, S, Vt


def ridge_svd_solve(U: np.ndarray, S: np.ndarray, Vt: np.ndarray,
                    Y: np.ndarray, lam: float) -> np.ndarray:
    """Ridge regression coefficient `xi(lam)` from the SVD of Phi_n.

    Solves `(Phi_n^T Phi_n + lam I) xi = Phi_n^T Y` using the closed form
    `xi = V diag(S / (S^2 + lam)) U^T Y`. `lam=0` reduces to the SVD
    pseudo-inverse (matches `np.linalg.lstsq` on the over-determined case).
    """
    filt = S / (S * S + lam)
    return Vt.T @ (filt[:, None] * (U.T @ Y))


def ridge_svd_press(U: np.ndarray, S: np.ndarray, Y: np.ndarray,
                    lam: float, *, h_floor: float = 1e-6) -> dict:
    """Leave-one-out PRESS for ridge regression via the SVD form.

    Hat matrix `H = U diag(S^2/(S^2+lam)) U^T`, so `h_ii =
    sum_k U_ik^2 S_k^2 / (S_k^2 + lam)`. Returns per-target sum of squared
    LOO residuals, the mean PRESS across targets, the count of rows where
    `1 - h_ii` had to be floored, and the effective d.o.f.
    """
    H_factor = (S * S) / (S * S + lam)
    h_diag = (U * U) @ H_factor
    Y_hat = U @ (H_factor[:, None] * (U.T @ Y))
    resid = Y - Y_hat
    denom = 1.0 - h_diag
    n_clipped = int(np.sum(denom < h_floor))
    denom = np.maximum(denom, h_floor)
    press_per_target = np.sum((resid / denom[:, None]) ** 2, axis=0)
    return dict(
        press_per_target=press_per_target,
        press=float(press_per_target.mean()),
        h_diag=h_diag,
        n_clipped=n_clipped,
        eff_dof=float(H_factor.sum()),
    )
...
def rbf_centers_hier_anisotropic_pca(
    ...
    *,
    ...
    normal_width_floor : relative floor on the normal half-width, expressed
        as a fraction of the smallest *tangent* half-width in the same
        cluster: `s_normal >= normal_width_floor * min_p sqrt(s2_tangent_p)`.
        Default 0.0 is an exact no-op. On a locally ~1-D attractor (KS_BUR)
        the centre cloud has near-zero normal spread, so the spacing rule
        collapses `s_normal` to `sqrt(eigenvalue_floor)`; the kernels then
        have no off-sheet support and a sub-percent normal drift switches
        every kernel off (Sigma_j Phi_j -> 0), killing the integrated field.
        Tying the floor to the in-cluster tangent scale gives every kernel
        a physically-scaled normal thickness without rewriting the global
        eigenvalue_floor (which would silently change L63/LOR96). Scale-
        relative and per-cluster by construction. rom-specialist 2026-06-22.
    ...
    if standardise:
        ...
        # Raw-space PCA: skip per-feature rescale. K-means, sample
        # covariance, and FPS all happen on centred raw coordinates, so
        # eigenvalues are physical variances and the PCA frame is the
        # standard energy-ordered POD frame of the cluster.
        sigma_A = np.ones(r, dtype=A.dtype)
    A_std = (A - mu_A) / sigma_A
    ...
    for k in range(K_shape):
        ...
        if r_tangent_k < r:
            ...
            # Relative floor tied to the in-cluster tangent scale. With the
            # default normal_width_floor=0.0 this is np.maximum(s_normal, 0)
            # on an already-positive array -> exact no-op.
            s_tan_min = np.sqrt(s2[:r_tangent_k]).min()
            s_normal = np.maximum(s_normal, normal_width_floor * s_tan_min)
    ...
    meta = {
        ...
        "normal_width_floor": float(normal_width_floor),
        "seed": int(seed),
...
def deriv_5point_full(A: np.ndarray, dt: float) -> np.ndarray:
    """5-point derivative at every sample, including the four boundary rows.

    Central 5-point stencil (O(dt^4)) on the interior indices 2..M-3, and
    one-sided 5-point stencils (also O(dt^4)) on indices 0, 1, M-2, M-1.
    Returns a (M, r) array aligned row-for-row with `A`, so the downstream
    SINDy fit sees a derivative for every snapshot. Use this in the
    pre-compute step where snapshots are at native dt and we want to retain
    every (a, a_dot) pair for arbitrary downstream striding.

    Boundary stencils (Fornberg 1988 coefficients, O(h^4) at the boundary):

        f'(x_0)     ~ [-25 f0 + 48 f1 - 36 f2 + 16 f3 -  3 f4] / (12 dt)
        f'(x_1)     ~ [ -3 f0 - 10 f1 + 18 f2 -  6 f3 +    f4] / (12 dt)
        f'(x_{M-2}) ~ [   -f_{M-5} + 6 f_{M-4} - 18 f_{M-3} + 10 f_{M-2} + 3 f_{M-1}] / (12 dt)
        f'(x_{M-1}) ~ [  3 f_{M-5} -16 f_{M-4} + 36 f_{M-3} - 48 f_{M-2} +25 f_{M-1}] / (12 dt)

    Parameters
    ----------
    A  : (M, r) snapshot array, M >= 5.
    dt : float, sample step.

    Returns
    -------
    dAdt : (M, r) derivative estimate at every row of A.
    """
    M = A.shape[0]
    if M < 5:
        raise ValueError("deriv_5point_full requires at least 5 snapshots")
    dAdt = np.empty_like(A, dtype=np.float64)
    inv12dt = 1.0 / (12.0 * dt)
    dAdt[2:-2] = (-A[4:] + 8.0 * A[3:-1] - 8.0 * A[1:-3] + A[:-4]) * inv12dt
    dAdt[0] = (-25.0 * A[0] + 48.0 * A[1] - 36.0 * A[2]
               + 16.0 * A[3] - 3.0 * A[4]) * inv12dt
    dAdt[1] = (-3.0 * A[0] - 10.0 * A[1] + 18.0 * A[2]
               - 6.0 * A[3] + A[4]) * inv12dt
    dAdt[-2] = (-A[-5] + 6.0 * A[-4] - 18.0 * A[-3]
                + 10.0 * A[-2] + 3.0 * A[-1]) * inv12dt
    dAdt[-1] = (3.0 * A[-5] - 16.0 * A[-4] + 36.0 * A[-3]
                - 48.0 * A[-2] + 25.0 * A[-1]) * inv12dt
    return dAdt
