...
#
# Extend the MFU_NA d_o=45 alpha_tangent sweep to resolve the drag-spectrum
# optimum: the premultiplied f*PSD of the mean skin friction matches the FOM
# best near alpha=8 (shape-TV minimum), but the existing grid (3.5,5,8,12) only
# brackets it. Fits the six infill/extension alphas with the same config and
# N-grid as launch_mfu_na_rbf_alpha_sweep_savemodels.sh DO=45; existing alphas
# untouched.
#
#   sbatch scripts/launch_mfu_na_rbf_alpha_sweep_do45_extend.sh
#
# Writes results/MFU_NA/do45/alpha<A>/{sweep.npz, model_n*.npz, rbf_K16_n_sweep.png}
...
ALPHA_VALUES=(4.0 6.0 7.0 10.0 16.0 20.0)
...
for A in "${ALPHA_VALUES[@]}"; do
    ...
    python -u scripts/run_rbf_K_n_sweep.py \
        ...
        --title "MFU_NA d_o=${DO} K=${K} alpha_tangent=${A} (extend)" \
        > "${OUT}/sweep.log" 2>&1 &
    pids+=($!)
done
